...
class SearchRequest(BaseModel):
    ...
    type: Optional[str] = Field(None, description="媒體類型: audio/video/document/image")
...
app = FastAPI(
    title="音頻相似度搜索 API",
    description="基於 Qdrant 的音頻內容相似度搜索服務",
    version="1.0.0"
)
...
@app.post("/audio_search", response_model=SearchResponse, tags=["搜索"])
async def search_audio(request: SearchRequest):
    ...
    try:
        ...
        print(f"[TIMED] /audio_search took {end - start:.3f}s")
...
@app.get("/collection/stats", tags=["集合管理"])
async def get_collection_stats():
    ...
    try:
        ...
        embedding_types, media_types = {}, {}
        filenames, speakers = set(), set()
        for point in points:
            p = point.payload
            et = p.get("embedding_type")
            if et: embedding_types[et] = embedding_types.get(et, 0) + 1
            mt = p.get("type")
            if mt: media_types[mt] = media_types.get(mt, 0) + 1
            fn = p.get("filename")
        ...
        return {
            ...
            "sample_files": sorted(list(filenames))[:10],
...
if __name__ == "__main__":
    uvicorn.run(
        "api_audio_search_with_cache:app",
        host="0.0.0.0",
        port=8812,
        reload=True
    )
