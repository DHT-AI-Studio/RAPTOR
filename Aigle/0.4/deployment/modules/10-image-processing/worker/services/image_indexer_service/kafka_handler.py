# services/image_indexer_service/kafka_handler.py
...
class ImageIndexerKafkaHandler:
    def __init__(self):
        self.consumer = None
        self.producer = None
    ...
    async def process_message(self, message: Dict[str, Any]):
        ...
        merged_result_path = parameters.get("merged_result_path")
        try:
            logger.info(f"Received message: {message.get('message_id')}")

            if not merged_result_path:
                raise ValueError("Missing merged_result_path in parameters")
            ...
            results = {}
            # results = await self.ingest_to_search(merged_result_path)

            # Personal DB side-output (VIE01-190). Before the `finally` block
            # deletes merged_result_path — that file is the only copy of the
            # entries here. Fire-and-forget: never fails this handler.
            await publish_personal_index(
                self.producer,
                source_module="10-image",
                entries_path=merged_result_path,
            )
            ...
            if merged_result_path:
                try:
                    if os.path.exists(merged_result_path):
                        os.remove(merged_result_path)
                except Exception as cleanup_err:
                    logger.warning(f"Failed to clean up merged result file {merged_result_path}: {cleanup_err}")
