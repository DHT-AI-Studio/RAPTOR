> **Note:** This directory is **not used in deployment**. It contains historical Redis configurations and the `cache_manager` library for reference only. The active Redis deployment is defined in `../docker-compose.yml`.
