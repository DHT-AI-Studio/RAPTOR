...
class ConfigUpdateRequest(BaseModel):
    config_path: List[str] = Field(..., description="配置路径，如 ['inference', 'engines', 'ollama', 'timeout']")
...
def get_inference_engines_config():
    """獲取推理引擎的配置信息（spec-driven 後僅剩 adapter 設定）"""
    try:
...
def validate_config():
    ...
    try:
        ...
        # 檢查推理配置（v3 只需要 engines 段）
        inference_config = config.get_config("inference")
        if inference_config:
            engines_cfg = inference_config.get("engines") or {}
            if not engines_cfg:
                validation_results["issues"].append("推理配置中缺少 engines 段")
                validation_results["overall_valid"] = False
            else:
                for required in ("ollama", "transformers"):
                    if required not in engines_cfg:
                        validation_results["warnings"].append(
                            f"engines.{required} 未設定（將使用 adapter 預設值）"
                        )
