...
ASYNC_PROCESSING_CONFIG = {
    "max_workers": int(os.getenv("DOCUMENT_ANALYSIS_MAX_WORKERS", "4")),
    "contextual_timeout": int(os.getenv("DOCUMENT_ANALYSIS_CONTEXTUAL_TIMEOUT", "180")),
...
# 臨時檔案配置
TEMP_FILE_DIR = "/tmp/media_processing/document_processing"
...
# VLM API 配置
# VLM_API_URL = "http://192.168.57.156:8002" 

# Contextual chunking config
CHUNK_TOKENS_BY_TYPE = {"pdf": 400, "office": 600, "spreadsheet": 200}
CONTEXTUAL_DOC_CHARS = 8000        # max chars of full document sent to LLM for context generation
CONTEXTUAL_BATCH_SIZE = int(os.getenv("DOCUMENT_ANALYSIS_CONTEXTUAL_BATCH_SIZE", "5"))  # chunks per LLM call

# 處理配置 (legacy — kept for backwards compatibility)
