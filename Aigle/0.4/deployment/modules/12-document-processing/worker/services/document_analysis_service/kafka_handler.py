# services/document_analysis_service/kafka_handler.py
...
class DocumentAnalysisKafkaHandler:
    def __init__(self, analysis_client=None, thread_pool=None):
        self.bootstrap_servers = KAFKA_BOOTSTRAP_SERVERS
        self.request_topic = KAFKA_TOPIC_ANALYSIS_REQUEST
        self.result_topic = KAFKA_TOPIC_ANALYSIS_RESULT
        self.dlq_topic = KAFKA_TOPIC_DLQ
        self.analysis_client = analysis_client or DocumentAnalysisClient()
        self.service_name = "document_analysis_service"

        self.thread_pool = thread_pool or concurrent.futures.ThreadPoolExecutor(
    ...
    async def start_consumer(self):
        ...
        consumer = AIOKafkaConsumer(
            ...
            group_id="document-analysis-service-group",
        ...
        logger.info("Document Analysis Kafka consumer started, waiting for messages...")
    ...
    async def process_message_async(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        async with self.task_semaphore:
            try:
                ...
                # 處理文件分析請求
                if message["payload"]["action"] == "document_analysis":
                    # 記錄任務開始
                    ...
                    try:
                        analysis_results = await self.handle_document_analysis_async(message)
                        
                        response_message = MessageBuilder.create_analysis_result_message(
    ...
    async def process_message(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        """原有的同步處理方法（備用）"""
        await self.process_message_async(message, producer)
    
    async def handle_document_analysis_async(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """異步處理文件分析請求"""
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    self.thread_pool,
                    self.analysis_client.analyze_document_sync,
                    message["payload"]
                ),
            ...
            logger.error(f"Document analysis timeout for message: {message.get('message_id')}")
            return {
                "request_id": message["payload"].get("request_id"),
    ...
    async def handle_document_analysis(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """處理文件分析請求（向後兼容）"""
        return await self.handle_document_analysis_async(message)
    ...
    def validate_message(self, message: Dict[str, Any]) -> bool:
        ...
        if payload.get("action") == "document_analysis":
            ...
            if "temp_file_path" not in parameters:
                logger.error("Missing temp_file_path in parameters")
                return False
