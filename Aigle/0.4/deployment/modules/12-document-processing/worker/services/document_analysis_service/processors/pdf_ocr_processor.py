...
# # If there is absolutely no readable text in the image, respond with exactly: 'NO_TEXT_FOUND'"""
    ...
    def _process_single_page_bytes(self, image_bytes: bytes, page_num: int) -> dict:
        ...
        # 合併內容並轉繁體
        combined_text = self.cc.convert(f"{extracted_text} <IMAGE>{description}</IMAGE>".strip())
    ...
    def process_document(self, pdf_path: str) -> dict:
        ...
        try:
            ...
            full_text = "\n\n".join(p["content"] for p in page_data_list)
            return {"chunk": chunks, "full_text": full_text}
