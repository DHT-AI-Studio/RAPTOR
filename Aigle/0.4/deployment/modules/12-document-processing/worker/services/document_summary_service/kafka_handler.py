...
INFERENCE_MODEL = os.getenv("INFERENCE_MODEL", "qwenforsummary")
INFERENCE_THINK = os.getenv("INFERENCE_THINK", "false").lower() == "true"
...
class DocumentSummaryKafkaHandler:
    ...
    async def process_message_async(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        async with self.task_semaphore:
            try:
                ...
                # Must go through send_error_response (result topic), not
                # send_to_dlq (KAFKA_TOPIC_DLQ) — nothing consumes the DLQ
                # topic, so a failure sent there vanishes silently instead of
                # reaching whatever is waiting for this job's result.
                await self.send_error_response(producer, message, str(e), "PROCESSING_ERROR")
                # 清理任務記錄
                self.active_tasks.pop(message_id, None)
    ...
    def summarize_document_sync(self, request_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            summary_result = self.summarizer.summarize_from_json(analysis_result_path)
            processing_time = (datetime.now() - start_time).total_seconds()

            summary_text = str(summary_result)

            # 檢查摘要結果
            if not summary_text or summary_text.startswith("處理文件時發生錯誤") or summary_text.startswith("摘要生成失敗"):
                return {
                    "request_id": request_id,
                    "action": "document_summary",
                    "status": "error",
            ...
            summary_result_path = os.path.join(
                SUMMARY_RESULTS_DIR,
                f"summary_result_{request_id}.json"  
            )
            try:
                # 讀取原始 JSON 文件
                ...
                new_summary_entry = {
                    ...
                    "payload": {
                        "filename": filename,
                        # Every other media type's summary entry sets this
                        # (image_orchestrator_service/message_utils.py,
                        # audio_orchestrator_service/kafka_handler.py) -- this
                        # one never did, so Module 25's document summary
                        # Chunks carried source=null while the per-chunk text
                        # entries right next to them had it set correctly.
                        "source": filename.rsplit(".", 1)[-1].lower() if "." in filename else "unknown",
                        "type": "documents",
            ...
            # Clean up analysis result file now that summary JSON contains all data
            try:
                os.remove(analysis_result_path)
                logger.info(f"[SYNC] Cleaned up analysis result file: {analysis_result_path}")
            except Exception as cleanup_err:
                logger.warning(f"Failed to cleanup analysis result file: {cleanup_err}")
            ...
            try:
                ...
                _params = request_payload.get('parameters', {})
                _analysis = _params.get('analysis_result', {}).get('results', {}).get('parameters', {})
                _analysis_path = _analysis.get('analysis_result_path')
                if _analysis_path and os.path.exists(_analysis_path):
                    os.remove(_analysis_path)
                    logger.info(f"Cleaned up analysis result file after error: {_analysis_path}")
                filename = _analysis.get('filename', 'unknown')
                summary_result_path = os.path.join(
                    SUMMARY_RESULTS_DIR,
                    f"summary_result_{request_id}.json"
                )
