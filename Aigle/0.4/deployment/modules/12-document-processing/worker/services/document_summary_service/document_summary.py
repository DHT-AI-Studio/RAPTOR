...
class _TransientInferenceError(Exception):
    """Raised on an empty model response (other than a guardrail content-
    policy block, see _ContentPolicyBlockedError below) -- more likely
    transient contention on the shared Ollama instance (also used by other
    media types' summary/analysis services and Module 25's personal-graph
    extraction) than a permanent failure. Caught at call_ollama()'s
    boundary and converted back to the existing string-return contract;
...
class _ContentPolicyBlockedError(Exception):
    ...
    the exception types @retry below watches for. Caught at call_ollama()'s
    boundary same as _TransientInferenceError, just without the pointless
    3x retry first."""


def _is_failed_summary(text: str) -> bool:
    """True if `text` is one of this service's own error-marker strings
    (see call_ollama()/summarize_from_json()'s early-return branches)
    rather than a real generated summary. summarize_from_json()'s
    multi-layer loop checks this after EVERY chunk-group/layer call and
    skips (does not append) a hit, rather than feeding it into the next
    layer's create_higher_level_prompt()/create_final_prompt() -- an LLM
    would otherwise happily paraphrase it into a fluent-looking paragraph
    that no longer starts with a recognizable error prefix, silently
    defeating this same check further up the chain. One failed/blocked
    group doesn't fail the whole document; only running out of groups
    entirely at some layer does (see the `if not summaries/new_summaries`
    checks right after each loop)."""
    return (text.startswith("錯誤:") or text.startswith("摘要生成失敗")
            or text.startswith("處理文件時發生錯誤") or text.startswith("JSON解析錯誤")
            or text.startswith("FORMAT ERROR"))

class DocumentSummarizer:
    def __init__(self, inference_url: str = "http://raptor-ai-lifecycle-api:8010", model_name: str = "qwenforsummary", think: bool = False):
    ...
    def call_ollama(self, prompt: str, system_prompt: str = "") -> str:
        """調用Ollama API"""
        url = f"{self.inference_url}/inference/infer"
        ...
        try:
            model_response = self._call_inference(url, payload)
            ...
            # _is_failed_summary()'s check still catches it.
            error_msg = f"摘要生成失敗: {str(e)}"
            print(f"摘要生成被 guardrail 擋下: {error_msg}")
            return error_msg
        except (requests.exceptions.Timeout, requests.exceptions.RequestException,
                requests.exceptions.HTTPError, _TransientInferenceError) as e:
            error_msg = f"摘要生成失敗: {str(e)}"
    ...
    @retry(
        ...
        retry=retry_if_exception_type((
            _TransientInferenceError, requests.exceptions.Timeout,
            requests.exceptions.RequestException, requests.exceptions.HTTPError,
        )),
        reraise=True,
    )
    def _call_inference(self, url: str, payload: Dict[str, Any]) -> str:
        """One inference call. Raises on anything retryable; returns the raw
        model response text on success."""
        ...
        model_response = result.get('result', {}).get('response', '').strip()
        if not model_response:
            print(f"警告: 未找到 response，完整回應: {result}")
                ...
                if current_layer == 1:
                    ...
                    for i, group in enumerate(groups):
                        ...
                        if _is_failed_summary(summary):
                            # Skip this group -- its error text must not be
                            # blended into a later layer's combine prompt
                            # (see _is_failed_summary()'s docstring) -- but
                            # other groups' content is unaffected, so keep
                            # going rather than failing the whole document.
                            print(f"第 {i+1}/{len(groups)} 組摘要失敗，略過此組: {summary}")
                            continue
                        summaries.append(summary)

                    if not summaries:
                        return "摘要生成失敗: 所有內容區塊均無法生成摘要"
                    ...
                    for i, group in enumerate(groups):
                        ...
                        summary = self.call_ollama(prompt, system_prompt)
                        if _is_failed_summary(summary):
                            print(f"第{current_layer}層第 {i+1}/{len(groups)} 組摘要失敗，略過此組: {summary}")
                            continue
