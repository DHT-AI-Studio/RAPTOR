...
class FrameDescriptionKafkaHandler:
    def __init__(self, processor=None):
        ...
        self.processor = processor or FrameDescriptionProcessor()
        # 確保結果目錄存在
    ...
    async def handle_frame_description_request(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        try:
            ...
            if moments:
                # Per-moment VLM descriptions using batch_chat
                logger.info(f"Using per-moment VLM for {len(moments)} moments")
                result_file_path = await self.processor.process_moments_descriptions(
                    moments=moments,
    ...
    async def send_error_response(
        ...
        try:
            ...
            # Must go to KAFKA_TOPIC_RESPONSE, not KAFKA_TOPIC_DLQ — nothing
