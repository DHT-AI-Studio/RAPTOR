...
class MessageBuilder:
    ...
    def create_summary_result_message(
        ...
        summarize_video_sync()/handle_video_summary() already set it correctly
        ("success"/"error") on every return path, but this envelope used to
        always say "success" regardless, so a downstream consumer that (as
        intended) trusts the top-level status instead of digging into the
        nested results.status would index a failed/blocked summary as if it
        were real content."""
        payload = {
