...
class VideoSummaryKafkaHandler:
    ...
    async def process_message(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        try:
            ...
            # send_to_dlq (KAFKA_TOPIC_DLQ) — nothing consumes the DLQ topic,
            # so a failure sent there vanishes silently instead of reaching
            # whatever is waiting for this job's result.
    ...
    def summarize_video_sync(self, request_payload: Dict[str, Any], kafka_message: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # video_analysis_path 必須，audio_analysis_path 允許為 None（無音軌影片）
            if not video_analysis_path:
                return {
                    "request_id": request_id,
                    "action": "video_summary",
                    "status": "error",
                    "error": {
                        "code": "MISSING_VIDEO_ANALYSIS_PATH",
                        "message": "Missing video analysis file path",
                        "timestamp": datetime.now().isoformat()
                    }
                }
            ...
            if audio_analysis_path and not os.path.exists(audio_analysis_path):
                return {
                    "request_id": request_id,
                    "action": "video_summary",
                    "status": "error",
            ...
            try:
                ...
                converted_audio_data = self.convert_audio_data_to_standard_format(
                    original_audio_data, filename, asset_path, version_id, status, branch_id
                )
                ...
                # analysis files are cleaned up by video_indexer_service after indexing
    ...
    def convert_audio_data_to_standard_format(self, original_audio_data: list, filename: str, asset_path: str, version_id: str, status: str, branch_id: str = "") -> list:
        """將原始音頻數據轉換為標準格式"""
        converted_data = []
