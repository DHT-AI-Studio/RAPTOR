...
import logging
import os
# import tempfile  -- only used by the retired HYBRID_SEARCH_INGEST_URL POST in ingest_to_search()
import uuid
# import aiohttp  -- only used by the retired HYBRID_SEARCH_INGEST_URL POST in ingest_to_search()
from aiokafka import AIOKafkaConsumer, AIOKafkaProducer
from datetime import datetime
from typing import Any, Dict, List, Optional
import config
from message_utils import create_response_message
from personal_index_publisher import publish_personal_index
...
class VideoIndexerKafkaHandler:
    ...
    async def process_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            # Personal DB side-output (VIE01-190). Unlike 09/10 this module
            # holds the whole-asset fields directly, so they are passed rather
            # than sniffed out of the entries. Fire-and-forget: never fails
            # this handler.
            #
            # entries=summary_entries (not entries_path=summary_result_path)
            # -- unlike 09/10/12 (where entries_path points at the exact same
            # enriched entries that went to Module 17), summary_result_path
            # for video is raw, pre-enrichment content: a mix of the summary
            # entry and one entry per individual ASR sentence, never itself
            # sent to 17 (ingest_to_search() reads it but writes the real
            # enriched content -- summary + moment_text_entries -- to a
            # different path). Passing the whole file as entries_path
            # published every one of those raw per-sentence entries as a
            # second, duplicate Chunk per sentence alongside the real
            # per-moment Chunk from `moments`, for every video ever indexed
            # since VIE01-190 -- fixed by dropping entries_path entirely.
            # That fix was too broad: summary_result_path's first entry is
            # the one genuinely valuable thing in that file (the whole-video
            # summary, embedding_type="summary" -- video_summary_service
            # always prepends it), and dropping entries_path took that with
            # it. ingest_to_search() already extracts just this entry via
            # _enrich_summary() for the Module 17 path; reuse that same
            # result (now returned as summary_entries) here instead of
            # re-deriving it a second time.
            await publish_personal_index(
                self.producer,
                source_module="11-video",
                entries=results.get("summary_entries", []),
                moments=contextualized_moments or chunking_moments,
                branch_id=branch_id,
    ...
    async def ingest_to_search(
        ...
        Module 17 (the actual HYBRID_SEARCH_INGEST_URL upload) retired -- the
        local enrichment steps below are kept as-is, not just commented out
        with the rest, because their outputs are still real dependencies of
        callers that are NOT retired: process_message() reads
        results["summary_entries"] to feed publish_personal_index() (module
        25's side-output), and video_graph_service used to read
        results["records_path"] the same way module 20's own consumer did
        (also commented out now, in that service's own kafka_handler.py --
        see that file for why records_path is still written here regardless:
        cheap local file write, no reason to also touch this function's
        contract for a module that isn't even reading it anymore).
        """
        try:
            ...
            return {
                ...
                "text_entries": moment_text_entries[:100],
                "summary_entries": summary_entries,
            }

            # if not HYBRID_SEARCH_INGEST_URL:
            #     logger.info("[DEBUG] HYBRID_SEARCH_INGEST_URL not set — skipping upload (dry-run)")
            #     return {"dry_run": True, "total_entries": len(enriched), "debug_file": debug_path, "records_path": records_path, "text_entries": moment_text_entries[:100], "summary_entries": summary_entries}
            #
            ...
            #                 return {**result.get("results", {}), "records_path": records_path, "text_entries": moment_text_entries[:100], "summary_entries": summary_entries}
            # finally:
            #     os.unlink(tmp_path)
