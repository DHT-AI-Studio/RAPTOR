...
#
# Consumed video-indexer-results and forwarded enriched records to module 20
# (graph-service) POST /ingest/payload. Module 20 retired -- personal-db (25)
# does its own independent extraction now, see _process() below; this service
# just drains + cleans up records_path so nothing accumulates on disk.
...
# import asyncio  -- only used by the retired _post_to_graph() below (polling sleep)
# import aiohttp  -- only used by the retired _post_to_graph() below
...
# GRAPH_SERVICE_URL = os.getenv("GRAPH_SERVICE_URL", "http://raptor-graph-service:8843")  -- module 20 retired
...
class VideoGraphKafkaHandler:
    ...
    async def _process(self, message: Dict[str, Any]):
        ...
        try:
            ...
            # Module 20 (graph-service) retired -- personal-db (25) does its
            # own independent extraction from the personal-index-requests
            # topic (see 25's kafka_consumer.py), not from records_path, so
            # nothing downstream needs this file's content any more. Still
            # read + delete it (not just delete) to keep the same log/observed
            # behavior as before for anyone watching this service's logs, and
            # to avoid records_path files accumulating on disk with nothing
            # left to clean them up now that _post_to_graph() below never
            # gets there to do it as a side effect.
            ...
            logger.info(f"[{message_id}] module 20 retired, not forwarding {len(records)} "
                       f"records for {version_id}")
            ...
            await self._send(
                ...
                _build_response(message, "success", {
                    ...
                    "records_sent": 0,
                    "skipped": "module 20 (graph-service) retired",
                }),
            )
            ...
            # Must go to KAFKA_TOPIC_RESULT, not KAFKA_TOPIC_DLQ — nothing
            # consumes the DLQ topic, so a failure sent there vanishes silently
            # instead of reaching whatever is waiting for this job's result.
            await self._send(
                config.KAFKA_TOPIC_RESULT,
