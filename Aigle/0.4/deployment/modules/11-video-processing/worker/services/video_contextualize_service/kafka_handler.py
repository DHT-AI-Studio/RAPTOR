...
class VideoContextualizeKafkaHandler:
    ...
    async def process_message(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        try:
            ...
            # Must go through _send_error (KAFKA_TOPIC_RESULT), not _send_to_dlq
            # (KAFKA_TOPIC_DLQ) — nothing consumes the DLQ topic, so a failure
            # sent there vanishes silently instead of reaching whatever is
            # waiting for this job's result.
            await self._send_error(producer, message, str(exc), "PROCESSING_ERROR")
