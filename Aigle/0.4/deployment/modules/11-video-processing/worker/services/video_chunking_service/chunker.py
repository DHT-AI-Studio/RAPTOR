...
from config import (
    ...
    NORMALIZE_PIX_FMT, NORMALIZE_TIMEOUT_SEC,
    NORMALIZE_GPU_RETRIES, NORMALIZE_GPU_RETRY_DELAY_SEC,
)
...
def _run_gpu_tier(cmd: List[str], tier_name: str) -> Optional[subprocess.CompletedProcess]:
    """Run one GPU-tier ffmpeg command, retrying up to NORMALIZE_GPU_RETRIES
    times (sleeping NORMALIZE_GPU_RETRY_DELAY_SEC between attempts) before
    giving up. GPU failures like CUDA_ERROR_NO_DEVICE / OpenEncodeSessionEx
    can be transient (driver/toolkit hiccup, momentary contention) -- a
    cheap retry here can avoid falling through to the much slower full-CPU
    tier for something that would have succeeded a few seconds later.
    Returns the last CompletedProcess (success or failure), or None if every
    attempt timed out."""
    result = None
    for attempt in range(1, NORMALIZE_GPU_RETRIES + 1):
        try:
            result = subprocess.run(cmd, capture_output=True, timeout=NORMALIZE_TIMEOUT_SEC)
        except subprocess.TimeoutExpired:
            result = None
        if result is not None and result.returncode == 0:
            return result
        if attempt < NORMALIZE_GPU_RETRIES:
            logger.warning(
                f"{tier_name} attempt {attempt}/{NORMALIZE_GPU_RETRIES} failed, "
                f"retrying in {NORMALIZE_GPU_RETRY_DELAY_SEC}s: "
                f"{result.stderr.decode(errors='replace') if result else 'timed out'}"
            )
            time.sleep(NORMALIZE_GPU_RETRY_DELAY_SEC)
    return result
...
def normalize_video(video_path: str, request_id: str) -> str:
    ...
    if not rotate_filter and codec in _CUDA_DECODE_CODECS:
        ...
        result = _run_gpu_tier(cmd, "Full-GPU normalization")
        if result is not None and result.returncode == 0:
            logger.info(f"Video normalized to 720p H.264: {out_path}")
            return out_path
        logger.warning(
            f"Full-GPU normalization failed after {NORMALIZE_GPU_RETRIES} attempt(s), "
            f"trying CPU decode + nvenc: "
            f"{result.stderr.decode(errors='replace') if result else 'timed out'}"
        )
    ...
    result = _run_gpu_tier(cmd, "CPU decode + nvenc normalization")
    if result is not None and result.returncode == 0:
        logger.info(f"Video normalized to 720p H.264: {out_path}")
        return out_path

    # Fallback 2: full CPU (libx264) -- not retried; both GPU tiers already were.
    logger.warning(
        f"CPU decode + nvenc failed after {NORMALIZE_GPU_RETRIES} attempt(s), "
