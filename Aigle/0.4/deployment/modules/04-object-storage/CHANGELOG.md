...
### Added
- **`database.py`**: Added `get_assets_by_branch(branch)` query for bulk asset lookup by branch, used in test teardown cleanup.

### Changed
- **Search Sync Refactor (Qdrant-only → Qdrant + OpenSearch dual sync)**:
    - Replaced `vector_store.py` (`VectorStore`) with `search_sync.py` (`SearchSync`) to synchronize asset lifecycle events to both **Qdrant** (`raptor` collection) and **OpenSearch** (`hybrid_index`), aligning with the `17-hybrid-search` module architecture.
    - `SearchSync` exposes the same public interface as the old `VectorStore` (`archive_metadata`, `delete_metadata`, `reactivate_metadata`, `clone_point`), requiring no changes to `client.py` call sites.
    - All lifecycle operations (`archive`, `destroy`, `clone`) now propagate to both stores concurrently via `asyncio.gather`.
    - `clone_point` now clones documents in both Qdrant (scroll + upsert) and OpenSearch (search_after pagination + async_bulk).
    - `initialize()` now performs connection checks against both stores instead of creating collections.
- **`config.py`**: Added `qdrant_collection`, `opensearch_host`, `opensearch_port`, `opensearch_user`, `opensearch_password`, `opensearch_index`, `opensearch_verify_certs` settings.
- **`requirements.txt`**: Added `opensearch-py[async]==3.1.0`.
- **`docker-compose.yaml`**: Removed orphaned `qdrant_data` volume (Qdrant is now an external shared service).
- **`.env` / `.env.example`**: Updated Qdrant host to `raptor-qdrant` (shared raptor network alias); added `QDRANT_COLLECTION` and all `OPENSEARCH_*` environment variables.
- **`tests/conftest.py`**: Updated teardown to use `SearchSync`; replaced branch-based Qdrant filter cleanup with per-asset `delete_metadata` calls driven by PostgreSQL lookup.

### Removed
- `asset_management/vector_store.py`: Superseded by `search_sync.py`.
