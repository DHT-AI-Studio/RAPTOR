# from qdrant_client import AsyncQdrantClient  -- module 17 retired
# from qdrant_client.models import PointStruct, Filter, FieldCondition, MatchValue  -- module 17 retired
# from opensearchpy import AsyncOpenSearch  -- module 17 retired
# from opensearchpy.helpers import async_bulk  -- module 17 retired
import httpx
import uuid
import asyncio
import logging
...
class SearchSync:
    ...
    async def initialize(self):
        if self._initialized:
            return
        # Module 17 retired -- Qdrant/OpenSearch reachability checks removed
        # along with the clients above. Module 25 (ArcadeDB) has no
        # equivalent "confirm the collection/index exists" step here; its
        # per-user database is created on first use (POST /internal/db/init),
        # not checked eagerly at SearchSync startup.
        # try:
    ...
    async def close(self):
        ...
        # await self.opensearch.close()  -- module 17 retired
        self._initialized = False
        logger.info("[SearchSync] Connections closed.")

    # ---------- shared filter builders ----------
    # Module 17 retired -- only used by the commented-out Qdrant/OpenSearch
    # internals below.
    ...
    async def check_indexed(self, asset_path: str, version_id: str, branch: str) -> bool:
        """Return True if this asset version has been indexed in Module 25
        (i.e. analysis completed). Was Qdrant-only (module 17 retired) --
        that version always returned True on any error, silently skipping
        re-analysis of content that was never actually indexed anywhere;
        kept the same fail-open behavior here since the calling code's
        contract ("assume indexed, don't spuriously re-process") hasn't
        changed, just which store answers the question."""
        url = settings.personal_db_service_url
        if not url or not branch:
            return True
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.get(
                    f"{url}/personal/index/{version_id}/exists",
                    headers={"X-Branch-ID": branch},
                )
                resp.raise_for_status()
                return bool(resp.json().get("indexed", False))
        except Exception as e:
            logger.warning(f"[PersonalDB] check_indexed failed for {asset_path}/{version_id}: {e}")
    ...
    async def archive_metadata(self, asset_path: str, version_id: str, branch: str):
        await asyncio.gather(
            ...
            # -- modules 17/20 retired
            self._personal_db_set_status(asset_path, version_id, "archived", branch_id=branch),
        )

    async def reactivate_metadata(self, asset_path: str, version_id: str, branch: str):
        """Not currently called anywhere -- client.py's archive flow is
        one-way (see its own "Asset is already archived and cannot be
        reactivated or modified" check), so nothing in this codebase
        reaches this method today. Kept, not deleted, in case that changes;
        updated the same way archive_metadata()/delete_metadata() were so
        it stays consistent if it's ever wired up."""
        await asyncio.gather(
            ...
            # -- modules 17/20 retired
            self._personal_db_set_status(asset_path, version_id, "active", branch_id=branch),
        )
    ...
    async def delete_metadata(self, asset_path: str, version_id: str, branch: str):
        await asyncio.gather(
            ...
            # -- modules 17/20 retired
            self._personal_db_delete(asset_path, version_id, branch_id=branch),
        )
    ...
    async def clone_point(
        ...
        await self._personal_db_clone(source_path, source_version, target_path, target_version, branch_id=branch)
        # await asyncio.gather(
    ...
    # Module 20 retired -- kept for rollback, not deleted.
    ...
    # ---------- Personal DB Service (Module 25) internals ----------

    async def _personal_db_set_status(self, asset_path: str, version_id: str, status: str, branch_id: str = None):
        url = settings.personal_db_service_url
        if not url:
            return
        if not branch_id:
            logger.warning(f"[PersonalDB] Skipping status={status} for {asset_path}/{version_id}: no branch_id")
            return
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.post(
                    f"{url}/personal/index/{version_id}/status",
                    headers={"X-Branch-ID": branch_id},
                    json={"status": status},
                )
                resp.raise_for_status()
            logger.info(f"[PersonalDB] status={status} for {asset_path}/{version_id}")
        except Exception as e:
            logger.error(f"[PersonalDB] Failed to set status for {asset_path}/{version_id}: {e}")

    async def _personal_db_delete(self, asset_path: str, version_id: str, branch_id: str = None):
        url = settings.personal_db_service_url
        if not url:
            return
        if not branch_id:
            logger.warning(f"[PersonalDB] Skipping delete for {asset_path}/{version_id}: no branch_id")
            return
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                resp = await client.delete(
                    f"{url}/personal/index/{version_id}",
                    headers={"X-Branch-ID": branch_id},
                )
                resp.raise_for_status()
            logger.info(f"[PersonalDB] Deleted ArcadeDB nodes for {asset_path}/{version_id}")
        except Exception as e:
            logger.error(f"[PersonalDB] Failed to delete ArcadeDB nodes for {asset_path}/{version_id}: {e}")

    async def _personal_db_clone(
        self,
        source_path: str,
        source_version: str,
        target_path: str,
        ...
        """Module 25's equivalent of the retired _qdrant_clone()/
        _opensearch_clone() pair -- one call instead of two, since 25 already
        clones Source+Chunk+RELATION+TemporalFact together server-side (see
        indexer.clone_version()), not a per-store copy this caller has to
        drive point-by-point the way Qdrant/OpenSearch needed."""
        url = settings.personal_db_service_url
        if not url:
            return
        if not branch_id:
            logger.warning(f"[PersonalDB] Skipping clone {source_path} → {target_path}: no branch_id")
            return
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                resp = await client.post(
                    f"{url}/personal/index/clone",
                    headers={"X-Branch-ID": branch_id},
                    json={
                ...
                resp.raise_for_status()
            logger.info(f"[PersonalDB] Cloned {source_path}/{source_version} → {target_path}/{target_version}")
        except Exception as e:
            logger.error(f"[PersonalDB] Clone failed {source_path} → {target_path}: {e}")
            raise
