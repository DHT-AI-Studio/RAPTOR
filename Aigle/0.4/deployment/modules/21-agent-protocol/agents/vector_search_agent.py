...
A2A wrapper for vector search via Module 25 (raptor-personal-db-service).
Backend: POST /personal/search/vector (X-Branch-ID header, per-caller isolated DB)
Was Module 17 (raptor-hybridsearch-api) -- see the commented-out block below
_do_vector_search for the old implementation, kept for rollback.
"""
...
def _ensure_database(personal_db_url: str, branch_id: str) -> None:
    ...
    try:
        ...
        logger.warning("[VectorSearch] module 25 db init failed for {}: {}", branch_id, exc)
...
def _do_vector_search(req: VectorSearchRequest) -> SearchResult:
    ...
    url  = f"{personal_db_url}/personal/search/vector"
...
agent_card = AgentCard(
    ...
    description="Semantic vector search via Module 25 (BAAI/bge-m3, ArcadeDB, per-user isolated).",
    version="0.3.0",
