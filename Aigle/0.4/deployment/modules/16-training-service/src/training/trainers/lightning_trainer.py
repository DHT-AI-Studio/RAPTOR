...
class LightningTrainer(BaseTrainer):
    ...
    def __init__(self, config: TrainerConfig, cancel_token: Any = None, mlflow_uri: Optional[str] = None):
        ...
        # Seed every RNG before any model/dataloader is built so two runs of the
        # same config are reproducible (otherwise weight-init/dropout noise makes
        # identical hyperparameters score differently — see AutoTune re-run issue).
