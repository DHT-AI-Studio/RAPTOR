...
class LightningLLMModule(L.LightningModule):
    ...
    def _apply_lora(self, lora_config: Dict[str, Any]) -> None:        
        ...
        peft_config = LoraConfig(
            ...
            use_rslora=lora_config.get('use_rslora', False),  # rank-stabilized LoRA scaling
            use_dora=lora_config.get('use_dora', False),      # weight-decomposed LoRA
    ...
    def configure_optimizers(self) -> Dict[str, Any]:
        ...
        elif getattr(self.config, "optimizer_type", "adamw") == "sgd":
            logger.info("Using torch.optim.SGD (momentum=0.9).")
        ...
        elif getattr(self.config, "optimizer_type", "adamw") == "lion":
            import bitsandbytes as bnb
            logger.info("Using bnb.optim.Lion8bit.")
        ...
        # Scheduler — selectable via lr_scheduler_type (linear/cosine/constant).
        # "constant" still honours warmup (constant_with_warmup); unknown → linear.
        _sched_names = {"linear": "linear", "cosine": "cosine", "constant": "constant_with_warmup"}
        sched_name = _sched_names.get(getattr(self.config, "lr_scheduler_type", "linear"), "linear")
