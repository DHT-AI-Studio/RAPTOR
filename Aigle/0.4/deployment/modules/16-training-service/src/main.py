...
async def lifespan(app: FastAPI):
    ...
    # Reclaim GPU from training workers orphaned by a previous worker/container
    # restart (they outlive the job and keep holding VRAM). Acts only when idle
    # (no 'running' job) and only on GPU-holding training processes — see
    # orphan_reaper for the safety rules.
    try:
        from .core.orphan_reaper import reap_orphaned_gpu_workers
        from .core.training_job_manager import get_job_manager
        running = get_job_manager().list_jobs(status="running")
        reap_orphaned_gpu_workers(has_running_jobs=bool(running))
    except Exception as e:  # never let cleanup block startup
        logger.warning(f"Startup orphan reaper skipped: {e}")
