# services/audio_recognizer_service/kafka_handler.py
...
class AudioRecognizerKafkaHandler:
    def __init__(self, recognition_client=None, thread_pool=None):
        self.bootstrap_servers = KAFKA_BOOTSTRAP_SERVERS
        self.request_topic = KAFKA_TOPIC_RECOGNIZER_REQUEST
        self.result_topic = KAFKA_TOPIC_RECOGNIZER_RESULT
        self.dlq_topic = KAFKA_TOPIC_DLQ
        self.service_name = "audio_recognizer_service"

        self.recognition_client = recognition_client or AudioRecognitionClient()
        ...
        self.thread_pool = thread_pool or concurrent.futures.ThreadPoolExecutor(
            ...
            thread_name_prefix="AudioRecognition"
        )
        max_concurrent = 1 if not self._owns_thread_pool else ASYNC_PROCESSING_CONFIG["max_concurrent_tasks"]
    ...
    async def start_consumer(self):
        ...
        consumer = AIOKafkaConsumer(
            ...
            group_id="audio-recognizer-service-group",
        ...
        logger.info("Audio Recognizer Kafka consumer started, waiting for messages...")
    ...
    async def process_message_async(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        async with self.task_semaphore:
            try:
                ...
                # 處理音頻識別請求
                if message["payload"]["action"] == "audio_recognition":
                    # 記錄任務開始
    ...
    async def handle_audio_recognition_async(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """異步處理音頻識別請求"""
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    self.thread_pool,
                    self.recognition_client.recognize_audio_sync,
                    message["payload"]
                ),
            ...
            logger.error(f"Audio recognition timeout for message: {message.get('message_id')}")
            return {
                "request_id": message["payload"].get("request_id"),
    ...
    def validate_message(self, message: Dict[str, Any]) -> bool:
        ...
        if payload.get("action") == "audio_recognition":
