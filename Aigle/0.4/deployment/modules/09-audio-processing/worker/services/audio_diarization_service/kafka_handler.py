# services/audio_diarization_service/kafka_handler.py
...
class AudioDiarizationKafkaHandler:
    def __init__(self, diarization_client=None, thread_pool=None):
        self.bootstrap_servers = KAFKA_BOOTSTRAP_SERVERS
        self.request_topic = KAFKA_TOPIC_DIARIZATION_REQUEST
        self.result_topic = KAFKA_TOPIC_DIARIZATION_RESULT
        self.dlq_topic = KAFKA_TOPIC_DLQ
        self.diarization_client = diarization_client or AudioDiarizationClient()
        self.service_name = "audio_diarization_service"
        ...
        self.thread_pool = thread_pool or concurrent.futures.ThreadPoolExecutor(
            ...
            thread_name_prefix="AudioDiarization"
        )
    ...
    async def start_consumer(self):
        ...
        consumer = AIOKafkaConsumer(
            ...
            group_id="audio-diarization-service-group",
        ...
        logger.info("Audio Diarization Kafka consumer started, waiting for messages...")
    ...
    async def process_message_async(self, message: Dict[str, Any], producer: AIOKafkaProducer):
        ...
        async with self.task_semaphore:
            try:
                ...
                # 處理音頻說話人分離請求
                if message["payload"]["action"] == "audio_diarization":
                    # 記錄任務開始
    ...
    async def handle_audio_diarization_async(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """異步處理音頻說話人分離請求"""
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    self.thread_pool,
                    self.diarization_client.diarize_audio_sync,
                    message["payload"]
                ),
            ...
            logger.error(f"Audio diarization timeout for message: {message.get('message_id')}")
            return {
                "request_id": message["payload"].get("request_id"),
            ...
            logger.error(f"Audio diarization error: {e}")
    ...
    def validate_message(self, message: Dict[str, Any]) -> bool:
        ...
        if payload.get("action") == "audio_diarization":
