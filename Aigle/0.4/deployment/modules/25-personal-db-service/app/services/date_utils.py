# Deterministic date extraction (Fix 1B), ported from Module 20
# (graph-service)'s app/date_utils.py -- same regexes, same strategy.
#
...
def fill_relation_dates(
    ...
    25's entities carry name lookup via "name" directly on the relation dict
    (subject/object), not an id -> name map like Module 20's -- but relations
    here have already been resolved to subject_id/object_id by the time this
    runs (see graph_extractor.py's extract_entities_and_relations()), so the
    same id -> name lookup Module 20 uses applies unchanged.
    """
