...
# Module 07 (ai-lifecycle-api) -- unified inference gateway, same one module 21's
# _llm_answer() and 09/11/12's summary services already go through. Replaces
# the direct-to-Ollama-OpenAI-compat-layer calls below (still commented in,
# not deleted): that layer has no way to disable qwen3.x's "thinking" mode,
# which was confirmed live to add 10-20x latency for no change in the final
# answer (4.8s with think=false vs 59s with think=true on an identical
# extraction prompt) -- the likely cause of this service occasionally missing
# its 90s LLM-call timeout entirely on longer videos. Module 07's Ollama
# adapter defaults think=false and talks to Ollama's *native* /api/chat
# (proper chat-template handling for the system/user message split below),
# not the OpenAI-compat surface.
_INFERENCE_URL = os.getenv("INFERENCE_URL", "http://raptor-ai-lifecycle-api:8010")
_INFERENCE_THINK = os.getenv("INFERENCE_THINK", "false").lower() == "true"


async def _call_llm_via_07(messages: list, timeout: float = 90.0) -> str:
    async with httpx.AsyncClient(timeout=timeout) as client:
        resp = await client.post(
            f"{_INFERENCE_URL}/inference/infer",
            json={
                "task": "text-generation",
                "engine": "ollama",
                "model_name": _CHAT_MODEL,
                "data": {"messages": messages},
                "options": {"temperature": 0.0, "think": _INFERENCE_THINK},
            },
        )
...
async def extract_entities_and_relations(
    ...
    try:
        content = await _call_llm_via_07([
        ...
        # type(e).__name__ because httpx timeout-family exceptions often
        # stringify to "" -- "LLM extraction failed ()" told us nothing
        # about what actually happened when this fired live.
        logger.warning(f"LLM extraction failed ({type(e).__name__}: {e}), returning empty results")
...
async def extract_moment_entities_batched(
    ...
    for start in range(0, len(targets), batch_size):
        ...
        try:
            ...
            logger.warning(f"Batched moment extraction failed for batch {start} "
                          f"({type(e).__name__}: {e})")
            continue
