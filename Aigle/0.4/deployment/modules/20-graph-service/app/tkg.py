...
# Module 07 (ai-lifecycle-api) -- see graph_builder.py's _call_llm_via_07 for
# the full rationale (think=false by default there, not on the OpenAI-compat
# layer this used to hit directly; measured 4.8s vs 59s for the same prompt).
...
async def llm_extract_temporal_facts(text: str, max_facts: int = 8) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(f"LLM extract failed ({type(e).__name__}: {e})")
        return []
