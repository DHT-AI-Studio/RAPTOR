...
| — module 11 (video)                                                            | **36 GB+ VRAM recommended** (ceiling, not re-verified against the current default — see note)                                                                      | InternVL frame description;`MAX_MEMORY_PER_GPU=36GiB` cap (shared with module 10). The default VLM was downsized from InternVL3.5-4B to **InternVL3.5-1B** (`VLM_MODEL_PATH` / `VIDEO_VLM_MODEL_PATH`) since the figure in this row was last measured, but `MAX_MEMORY_PER_GPU`/`VIDEO_FRAME_DESC_MEMORY_LIMIT` weren't correspondingly reduced and no re-verification on a smaller GPU is on record — treat 36 GB as an unconfirmed upper bound, not a floor, until it's re-tested (tracked in [issue #17](https://github.com/DHT-AI-Studio/RAPTOR/issues/17)) |
...
| `core-host`   | `10.0.0.20` | CPU infrastructure + APIs                   | 02, 03, 04, 05, 06, 13, (14), 15, 21, 22, 23, 24, 25, 26, 27 (skip 19/20 — deprecated) |
