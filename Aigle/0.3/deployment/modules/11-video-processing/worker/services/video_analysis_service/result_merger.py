...
class ResultMerger:
    ...
    async def merge_all_video_results(
        ...
        try:
            ...
            # 保存合併結果
            merged_filename = f"merged_video_analysis_{request_id}.json"
    ...
    async def _merge_all_results(
        ...
        moment_descriptions = frame_description_data.get("moment_descriptions", []) if isinstance(frame_description_data, dict) else []
        ...
        # 創建最終的合併結果，包含 event_summary 和 per-moment VLM descriptions
        merged_results = {
    ...
    async def merge_scene_and_ocr_results(
        ...
        try:
            ...
            # 保存合併結果
            merged_filename = f"merged_analysis_result_{request_id}.json"
            merged_file_path = os.path.join(MERGED_RESULTS_BASE_DIR, merged_filename)
