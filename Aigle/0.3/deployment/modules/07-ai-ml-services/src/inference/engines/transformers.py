...
class TransformersEngine(BaseEngine):
    ...
    def load_model(self, model_name: str, **kwargs) -> Any:
        ...
        try:
            ...
            # 2. 獲取任務類型（優先從 kwargs，否則從 options 推斷）
            task = kwargs.get('task', 'text-generation')
            
            # 3. 加載模型（傳遞所有 kwargs，包括 quantization，但排除 task 避免重複）
            load_kwargs = {k: v for k, v in kwargs.items() if k != 'task'}
            model = self._load_model_by_task(model_path, task, **load_kwargs)
    ...
    def _load_model_by_task(self, model_path: str, task: str, **kwargs) -> Any:
        ...
        # 對於 VLM、video、document 等任務，需要特殊處理
            ...
            # 從 kwargs 中獲取量化參數
            quantization = kwargs.pop('quantization', None)  # None, "4bit", or "8bit" (pop to avoid duplicate)
            return self._load_multimodal_model(model_path, task, device, quantization=quantization, **kwargs)
    ...
    def _load_multimodal_model(self, model_path: str, task: str, device: str, quantization: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """
        加載多模態模型
        ...
        Args:
            ...
            task (str): 任務類型
            device (str): 設備
            quantization (str, optional): 量化類型，可選值: "4bit", "8bit", None
            **kwargs: 額外參數
            ...
            if quantization:
                ...
                try:
                    ...
                    if quantization == "4bit":
                        ...
                        logger.info(f"使用 4-bit 量化加載模型: {model_path}")
                        ...
                        model_kwargs['quantization_config'] = quantization_config
                        model_kwargs['device_map'] = 'auto'
                        logger.info(f"使用 8-bit 量化加載模型: {model_path}")
                    else:
                        logger.warning(f"不支持的量化類型: {quantization}，使用標準加載")
                        model_kwargs['torch_dtype'] = self.torch_dtype
                except ImportError:
                    logger.warning("BitsAndBytesConfig 不可用，使用標準加載")
                    model_kwargs['torch_dtype'] = self.torch_dtype
            else:
                # 不使用量化，使用標準精度
                model_kwargs['torch_dtype'] = self.torch_dtype
            
            # 對於 Qwen2.5-VL，使用正確的模型類
            if 'qwen' in model_path.lower() and 'vl' in model_path.lower():
                try:
                    ...
                    logger.warning("Qwen2_5_VLForConditionalGeneration 不可用，使用 AutoModel")
                    model = AutoModel.from_pretrained(model_path, **model_kwargs)
            else:
                # 加載模型
                model = AutoModel.from_pretrained(model_path, **model_kwargs)
            
            # 如果沒有使用 device_map，手動移動到設備
    ...
    def _infer_vlm(self, model_dict: Dict[str, Any], inputs: Dict[str, Any], options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 檢查是否為 Qwen2.5-VL 模型（需要特殊處理）
            # 通過檢查 processor 類型或 model 類型來判斷
            processor_type = str(type(processor))
            model_type = str(type(model))
            is_qwen_vl = (
                'qwen' in processor_type.lower() and 'vl' in processor_type.lower()
            ) or (
                'qwen' in model_type.lower() and 'vl' in model_type.lower()
            ) or (
                hasattr(processor, 'apply_chat_template') and 
                hasattr(model, 'generate') and
                'Qwen2_5_VL' in model_type
            )
            
            if is_qwen_vl:
                # Qwen2.5-VL 需要特殊的消息格式和視覺處理
                try:
                    ...
                    logger.warning("qwen_vl_utils not available, using standard processing")
                    # 回退到標準處理
                    processed_inputs = processor(
                        images=image,
                        text=prompt,
            ...
            if is_qwen_vl:
                ...
                try:
                    generated_ids_trimmed = [
                        out_ids[len(in_ids):] for in_ids, out_ids in zip(processed_inputs.input_ids, outputs)
                    ]
