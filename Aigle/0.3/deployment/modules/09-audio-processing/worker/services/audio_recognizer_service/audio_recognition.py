    ...
    def recognize_audio_sync(self, request_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            recognition_result_path = os.path.join(
                RECOGNITION_RESULTS_DIR,
                f"recognition_result_{request_id}.json"
            )
            
            # 保留 segments，並保留每個 segment 的 words（word-level timestamps）
            # 同時將簡體轉繁體
            result_data = [
                {
                    "start": seg.get("start"),
                    "end": seg.get("end"),
                    "text": self.cc.convert(seg.get("text", "").strip()),
                    "words": [
                        {**w, "word": self.cc.convert(w.get("word", ""))}
                        for w in seg.get("words", [])
                    ],
                }
                for seg in recognition_result.get("segments", [])
            ]
            ...
            # 返回成功結果
            full_text = self.cc.convert(recognition_result.get("text") or "")
            return {
                "request_id": request_id,
                "action": "audio_recognition",
                "status": "success",
