...
async def query_endpoint(body: QueryRequest, request: Request):
    ...
    if body.mode == "tool":
        ...
        agent, rag_tool = build_tool_agent(user_id=user_id, branch_id=branch_id)
        answer = await asyncio.to_thread(agent.run, body.question)
        return {
            "answer":        _s2tw(str(answer)),
    ...
    result = await run_rag_pipeline(body.question, top_k=body.top_k, user_id=user_id, branch_id=branch_id)
