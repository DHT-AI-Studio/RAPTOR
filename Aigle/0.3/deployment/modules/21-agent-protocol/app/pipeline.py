...
LLM answer → Module 07 /inference/infer
Intent     → Module 18 /classify (BM25+FAISS+LightGBM+LLM) → Ollama fallback → regex fallback
...
async def build_query_plan(question: str, ollama_url: str) -> QueryPlan:
    plan = await _build_plan_from_module18(question)
    if plan is not None:
        return plan
    llm = await _classify_intent_llm(question, ollama_url)
    if llm is not None:
...
def assemble_grounded_prompt(question: str, top_chunks: list[dict],
                              graph_context: str, doc_sources: list[dict]) -> str:
    chunk_text = "\n\n".join(
    ...
    return (
        ...
        "Answer the question using ONLY the provided context chunks and graph relationships.\n"
...
async def _get_card(url: str, client: httpx.AsyncClient) -> "AgentCard | None":
    ...
    if url not in _agent_card_cache:
        try:
            ...
            resp.raise_for_status()
            _agent_card_cache[url] = AgentCard(**resp.json())
        except Exception as exc:
...
async def _llm_answer(prompt: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            resp = await client.post(
                f"{module07}/inference/infer",
                json={
                    "task": "text-generation",
                    "engine": "ollama",
                    "model_name": model_name,
                    "data": {"inputs": prompt},
                    "options": {"max_new_tokens": 512, "temperature": 0.3},
                },
            )
            resp.raise_for_status()
            return resp.json().get("result", {}).get("generated_text", "").strip()
    except Exception as exc:
...
async def run_rag_pipeline(
    ...
    plan = await build_query_plan(question, ollama_url)
    logger.info(
    ...
    # ── Step 5: Resolve presigned URLs via Module 04 ─────────────────────────
    auth_headers: dict = {}
    if user_id:
