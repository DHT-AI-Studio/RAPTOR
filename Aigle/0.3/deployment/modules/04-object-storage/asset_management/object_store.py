...
class ObjectStore:
    def __init__(self):
        ...
        # Persistent session for LakeFS presigned URL requests — reuses TCP connections.
        self._lakefs_session = requests.Session()
    ...
    async def get_file(self, key: str, version_id: Optional[str] = None, return_file_content: bool = False) -> Dict:
        ...
        try:
            ...
            if not return_file_content:
                # Skip obj.stat() — callers that only need the URL don't require content_type,
                # and stat() is an extra LakeFS round-trip that doubles latency.
                return {
                    "content_type": None,
    ...
    async def generate_presigned_url(self, key: str, version_id: Optional[str] = None) -> str:
        ...
        Raises:
            HTTPException: If the repository does not exist or if there is a failure in generating
                the presigned URL.
        """
        cache_key = f"presign:{version_id}:{key}"
        try:
            if self._redis:
            ...
            response = await asyncio.to_thread(
                lambda: self._lakefs_session.get(url, timeout=8, allow_redirects=False)
            )
            if response.status_code in (301, 302, 303, 307, 308):
                presigned = response.headers.get("Location", "")
            else:
                response.raise_for_status()
                presigned = response.url
            parsed_internal = urlparse(presigned)
            ...
            if self._redis:
                try:
                    await self._redis.setex(cache_key, settings.presign_url_ttl, external_url)
                except Exception:
                    pass
