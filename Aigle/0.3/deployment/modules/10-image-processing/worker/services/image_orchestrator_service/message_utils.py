...
class MessageBuilder:
    ...
    def create_processing_request(
        ...
        """創建處理請求消息"""
        _orig_payload = original_message["payload"]
        _dl_headers = _orig_payload.get("asset_managemant_download_header", {})
        _branch_id = (
            _orig_payload.get("parameters", {}).get("branch_id")
            or _dl_headers.get("X-Branch-ID")
            or _orig_payload.get("branch_id")
            or ""
        )
        payload = {
            "request_id": str(uuid.uuid4()),
            "original_request_id": _orig_payload.get("request_id"),
            "user_id": _orig_payload.get("user_id"),
            "access_token": _orig_payload.get("access_token"),
            "branch_id": _branch_id,
            "action": action,
            "parameters": {
                **parameters,
                "temp_file_path": temp_file_path,
                "asset_path": _orig_payload["parameters"].get("asset_path"),
                "version_id": _orig_payload["parameters"].get("version_id"),
                "status": _orig_payload["parameters"].get("status"),
                "branch_id": _branch_id,
            },
            "metadata": {
                "orchestrator_request_id": _orig_payload.get("request_id"),
                "created_at": datetime.now(timezone.utc).isoformat(),
                "original_metadata": _orig_payload.get("metadata", {})
            }
        }
...
class ImageResultMerger:
    ...
    def merge_results(
        ...
        try:
            ...
            branch_id = original_params.get("branch_id") or original_message["payload"].get("branch_id") or original_message["payload"].get("user_id", "")
