...
class ImageProcessor:
    ...
    def process_image_sync(self, request_payload: Dict[str, Any]) -> Tuple[str, str]:
        """
        同步版本的圖片分析方法（用於線程池執行）
        ...
        Returns:
            (description_result_path, ocr_result_path) 兩個結果檔案路徑
        """
        ...
        try:
            ...
            if not temp_file_path:
                raise ValueError("Missing temp_file_path in parameters")
            ...
            logger.info(f"[SYNC] Processing image: {file_path.name}")
            
            # 並行執行 description 和 OCR 分析
            description_result_path = self.generate_description(temp_file_path)
            ocr_result_path = self.extract_text(temp_file_path)
            
            logger.info(f"[SYNC] Analysis completed: description={description_result_path}, ocr={ocr_result_path}")
            
            return description_result_path, ocr_result_path
            
        except Exception as e:
            logger.error(f"[SYNC] Error processing image: {e}")
            raise
