...
class VLMAnnotator:
    ...
    def _init_local_model(self):
        """初始化本地 InternVL 模型"""
        try:
            # 模型路徑和配置（從環境變數讀取，允許 docker-compose 控制 GPU）
            path = os.environ.get("VLM_MODEL_PATH", "OpenGVLab/InternVL3_5-4B")
...
class OfficeDocumentProcessor:
    ...
    def process_document(self, file_path: Path) -> Dict[str, List[Dict]]:
        ...
        for chunk_index, chunk_data in enumerate(text_chunks):
            chunk = {
                ...
                "payload": {
                    ...
                    "type": "documents",
                    "text": self.cc.convert(chunk_data["content"]),
        ...
        return {"chunk": chunks, "full_text": markdown_text}
